...
const HomePage = () => {
  ...
  const [inventorySummary, setInventorySummary] = useState({ totalQuantity: 0, totalValue: 0, totalWeight: 0 });
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-100 p-4">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mb-4">
        <Link to="/evacuation" className="quick-action-btn bg-green-500 text-white py-4 rounded-lg hover:bg-green-600 transition duration-300 flex justify-center items-center">
          Perform Evacuation With Invoice
        </Link>
        <Link to="/modify-batch" className="quick-action-btn bg-indigo-500 text-white py-4 rounded-lg hover:bg-indigo-600 transition duration-300 flex justify-center items-center">
          Modify An Existing Batch
        </Link>
        <Link to="/add-supplier" className="quick-action-btn bg-yellow-500 text-white py-4 rounded-lg hover:bg-yellow-600 transition duration-300 flex justify-center items-center">
          Add A New Supplier/Farmer
        </Link>
        <Link to="/register-cocoa" className="quick-action-btn bg-purple-500 text-white py-4 rounded-lg hover:bg-purple-600 transition duration-300 flex justify-center items-center">
          Receive New Batch
        </Link>
        <Link to="/receive-stock" className="quick-action-btn bg-red-500 text-white py-4 rounded-lg hover:bg-red-600 transition duration-300 flex justify-center items-center">
          Receive Beans To An Existing Batch
        </Link>
        <Link to="/batch-expense" className="quick-action-btn bg-black text-white py-4 rounded-lg hover:bg-blue-600 transition duration-300 flex justify-center items-center">
          Add Expenditure For A Particular Batch
        </Link>
        <Link to="/pending-payment" className="quick-action-btn bg-blue-600 text-white py-4 rounded-lg hover:bg-red-600 transition duration-300 flex justify-center items-center">
          Pending Payments
        </Link>
        <Link to="/create-expense" className="quick-action-btn bg-blue-600 text-white py-4 rounded-lg hover:bg-red-600 transition duration-300 flex justify-center items-center">
          Create Expense
        </Link>
      </section>
      ...
      <section className="bg-teal-500 text-white p-8 rounded-lg shadow-lg mb-4 animate__animated animate__fadeIn text-center hover:shadow-2xl transition duration-300">
        ...
        <p>Total Weight Of Bags Available: {inventorySummary.totalWeight} kg</p>
      </section>
      ...
      <section className="bg-green-500 text-white p-8 rounded-lg shadow-lg mb-4 animate__animated animate__fadeIn text-center hover:shadow-2xl transition duration-300">
        ...
        <p>Expenditures: Ghc{financialData.totalExpenditures}</p>
      ...
      <section className="bg-red-500 text-white p-8 rounded-lg animate__animated animate__fadeIn hover:shadow-2xl transition duration-300">
